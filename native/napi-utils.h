...
bool GetPattern(napi_env env, napi_value value, std::string* pattern,
                size_t maximum = kMaxPatternBytes);
...
template <typename T>
napi_status CreateTaggedExternalRaw(napi_env env, std::unique_ptr<T> value,
                                    const napi_type_tag* type_tag, napi_value* result) {
  const napi_status create_status =
      napi_create_external(env, value.get(), Finalize<T>, nullptr, result);
...
template <typename T>
bool CreateTaggedExternal(napi_env env, std::unique_ptr<T> value,
                          const napi_type_tag* type_tag, napi_value* result) {
...
template <typename T>
bool GetTaggedExternal(napi_env env, napi_value value, const napi_type_tag* type_tag,
                       const char* error_message, T** result) {
  bool matches = false;
